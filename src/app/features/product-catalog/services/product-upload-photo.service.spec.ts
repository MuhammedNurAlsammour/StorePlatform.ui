...
import { ProductUploadPhotoService } from './product-upload-photo.service';
import { throwError } from 'rxjs';

describe('ProductUploadPhotoService', () => {
  let service: ProductUploadPhotoService;
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ProductUploadPhotoService,
        { provide: AlertService, useValue: alertServiceSpy },
        { provide: 'baseUrl', useValue: environment.apiUrls.baseUrl }
      ]
    });
    service = TestBed.inject(ProductUploadPhotoService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  it('ProductUploadPhotoService servisi testi', () => {
    expect(service).toBeTruthy();
  });

  it('GET /OrderItems/GetAllOrderItems read endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/OrderItems/GetAllOrderItems`)
    );
  ...
  it('GET /OrderItems/GetAllOrderItems read endpointinde hata oluşursa uyarı', async () => {
  ...
  it('GET /OrderItems/GetByIdOrderItems readById endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/OrderItems/GetByIdOrderItems`)
    );
    expect(req.request.method).toBe('GET');
    req.flush(mockResponse);
    const data = await promise;
  ...
  it('GET /OrderItems/GetByIdOrderItems readById endpointinde hata oluşursa uyarı', async () => {
    const errorMsg = 'Server ile iletişim kurulamadı. Lütfen daha sonra tekrar deneyiniz.';
    const promise = service.readId({ id: '1' },true);
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/OrderItems/GetByIdOrderItems`)
  ...
  it('post /OrderItems/CreateOrderItems fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('post /OrderItems/CreateOrderItems fonksiyonu hata durumunda hata fırlatmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /OrderItems/UpdateOrderItems fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /OrderItems/UpdateOrderItems fonksiyonu hata durumunda alertService.error çağrılmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
