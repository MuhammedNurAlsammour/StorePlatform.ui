...
import { EmployeeDefinitionSelectService } from './employee-definition-select.service';
import { throwError } from 'rxjs';
import { environment } from 'projects/shared/src/environments/environment.development';

describe('EmployeeDefinitionSelectService', () => {
  let service: EmployeeDefinitionSelectService;
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        EmployeeDefinitionSelectService,
        { provide: AlertService, useValue: alertServiceSpy },
        { provide: 'baseUrl', useValue: environment.apiUrls.baseUrl }
      ]
    });
    service = TestBed.inject(EmployeeDefinitionSelectService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  it('EmployeeDefinitionSelectService servisi testi', () => {
    expect(service).toBeTruthy();
  });


  it('GET /Employee/GetEmployeeListForManager read endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Employee/GetEmployeeListForManager`)
    );
  ...
  it('GET /Employee/GetEmployeeListForManager read endpoint\'inde hata oluşursa uyarı', async () => {
