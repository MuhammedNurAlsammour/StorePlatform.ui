...
import { EmployeesDefinitionsService } from './employees-definitions.service';
import { environment } from 'projects/shared/src/environments/environment.development';
import { throwError } from 'rxjs';

describe('EmployeesDefinitionsService', () => {
  let service: EmployeesDefinitionsService;
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        EmployeesDefinitionsService,
        { provide: AlertService, useValue: alertServiceSpy },
        { provide: 'baseUrl', useValue: environment.apiUrls.baseUrl }
      ]
    });
    service = TestBed.inject(EmployeesDefinitionsService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  it('EmployeesDefinitionsService servisi testi', () => {
    expect(service).toBeTruthy();
  });

  it('GET /Employee/GetEmployeeList read endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Employee/GetEmployeeList`)
    );
  ...
  it('GET /Employee/GetEmployeeList read endpointinde hata oluşursa uyarı', async () => {
  ...
  it('GET /Employee/GetEmployeeById GET endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const promise = service.readId({ page: 1, size: 10 });
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Employee/GetEmployeeById`)
    );
    expect(req.request.method).toBe('GET');
    req.flush(mockResponse);
    const data = await promise;
  ...
  it('GET /Employee/GetEmployeeById GET endpointinde hata oluşursa uyarı', async () => {
    const errorMsg = 'Server ile iletişim kurulamadı. Lütfen daha sonra tekrar deneyiniz.';
    const promise = service.readId({ page: 1, size: 10 });
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Employee/GetEmployeeById`)
  ...
  it('post /Employee/InsertEmployee fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('post /Employee/InsertEmployee fonksiyonu hata durumunda hata fırlatmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /Employee/MegaUpdateEmployee fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /Employee/MegaUpdateEmployee fonksiyonu hata durumunda alertService.error çağrılmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
