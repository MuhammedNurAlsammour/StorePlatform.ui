...
import { InstitutionDefinitionsService } from './institution-definitions.service';

describe('InstitutionDefinitionsService', () => {
  let service: InstitutionDefinitionsService;
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        InstitutionDefinitionsService,
        { provide: AlertService, useValue: alertServiceSpy },
        { provide: 'baseUrl', useValue: environment.apiUrls.baseUrl }
      ]
    });
    service = TestBed.inject(InstitutionDefinitionsService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  it('InstitutionDefinitionsService servisi testi', () => {
    expect(service).toBeTruthy();
  });

  it('GET /Institution/GetInstitutionList read endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Institution/GetInstitutionList`)
    );
  ...
  it('GET /Institution/GetInstitutionList read endpointinde hata oluşursa uyarı', async () => {
  ...
  it('GET /Institution/GetInstitutionLowerList GET endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Institution/GetInstitutionLowerList`)
    );
    expect(req.request.method).toBe('GET');
    req.flush(mockResponse);
    const data = await promise;
  ...
  it('GET /Institution/GetInstitutionLowerList GET endpointinde hata oluşursa uyarı', async () => {
    const errorMsg = 'Server ile iletişim kurulamadı. Lütfen daha sonra tekrar deneyiniz.';
    const promise = service.readId({ page: 1, size: 10 },true);
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Institution/GetInstitutionLowerList`)
  ...
  it('post /Institution/InsertInstitution fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('post /Institution/InsertInstitution fonksiyonu hata durumunda hata fırlatmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /Institution/UpdateInstitutionById fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /Institution/UpdateInstitutionById fonksiyonu hata durumunda alertService.error çağrılmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
