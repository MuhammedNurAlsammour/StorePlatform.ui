...
describe('PaymentService', () => {
  ...
  it('PaymentService servisi testi', () => {
    expect(service).toBeTruthy();
  });

  it('GET /Payments/GetAllPayments read endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Payments/GetAllPayments`)
    );
  ...
  it('GET /Payments/GetAllPayments read endpointinde hata oluşursa uyarı', async () => {
  ...
  it('GET /Payments/GetByIdPayments readById endpointine istek atmalı ve veriyi döndürmeli', async () => {
    const mockResponse = {
      result: [{
        id: '1',
        name: 'Test',
    ...
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Payments/GetByIdPayments`)
    );
    expect(req.request.method).toBe('GET');
    req.flush(mockResponse);
    const data = await promise;
  ...
  it('GET /Payments/GetByIdPayments readById endpointinde hata oluşursa uyarı', async () => {
    const errorMsg = 'Server ile iletişim kurulamadı. Lütfen daha sonra tekrar deneyiniz.';
    const promise = service.readId({ id: '1' },true);
    const req = httpMock.expectOne(
      req => req.url.includes(`${environment.apiUrls.baseUrl}/Payments/GetByIdPayments`)
  ...
  it('post /Payments/CreatePayments fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('post /Payments/CreatePayments fonksiyonu hata durumunda hata fırlatmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /Payments/UpdatePayments fonksiyonu başarılı olmalı', () => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
  ...
  it('update /Payments/UpdatePayments fonksiyonu hata durumunda alertService.error çağrılmalı', (done) => {
    const mockRequest = {
      id: '1',
      countryName: 'Test',
      countryCode: 1,
